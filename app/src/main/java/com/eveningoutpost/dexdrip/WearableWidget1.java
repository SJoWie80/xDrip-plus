 ...
 */
public class WearableWidget1 extends AppWidgetProvider {
    ...
    private static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.wearable_widget1);
        Log.d(TAG, "Update widget signal received");

        //update the time/date
        Date d = new Date();
        SimpleDateFormat sdf = new SimpleDateFormat("EEEE");
        views.setTextViewText(R.id.textDay, sdf.format(d));
        sdf.applyPattern("HH:mm a");
        views.setTextViewText(R.id.textTime, sdf.format(d));
        sdf.applyPattern("MMMM d, yyyy");
        views.setTextViewText(R.id.textDate, sdf.format(d));

        //Add behaviour: open xDrip on click
        Intent intent = new Intent(context, Home.class);
        PendingIntent pendingIntent = PendingIntent.getActivity(context, 0, intent, 0);
        views.setOnClickPendingIntent(R.id.WearWidget1, pendingIntent);
    ...
    private static void displayCurrentInfo(AppWidgetManager appWidgetManager, int appWidgetId, Context context, RemoteViews views) {
        Date d = new Date();
        BgGraphBuilder bgGraphBuilder = new BgGraphBuilder(context, d.getTime() - 3*60*60*1000, d.getTime());
        ...
        if (lastBgreading != null) {
            ...
            try {
                ...
                views.setImageViewBitmap(R.id.widgetGraph, new BgSparklineBuilder(context)
                        ...
                        .setHeight(125).setWidth(200).showHighLine(showLines).showLowLine(showLines).showAxes().build());
